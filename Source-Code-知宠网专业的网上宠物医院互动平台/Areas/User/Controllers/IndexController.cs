...
namespace ZoomLaCMS.Areas.User.Controllers
{
    public class IndexController : Ctrl_Guest
    {
        ...
        public ActionResult Register_Submit()
        {
            ...
            if (SiteConfig.UserConfig.MaximumUser > 0)
            {
                int currentRegUsers = Sql.getnum(info.TbName);  // 从数据库中读取当前注册人数
                if (currentRegUsers >= SiteConfig.UserConfig.MaximumUser)
                {
                    function.WriteErrMsg("ExceedUserNums（超过系统允许的最大会员值）");
                    return null;
                }
            }
            ...
            info.Email = DataConvert.CStr(Request.Form["TxtEmail"]).Replace(" ", "");
            ...
            else if (SiteConfig.UserConfig.Reg_AnswerMust && string.IsNullOrEmpty(info.Answer)) { function.WriteErrMsg("问题答案不能为空！"); return null; }
            ...
            M_User_InviteCode utMod = null;
            {
                ...
                if (DataConvert.CLng(info.ParentUserID) < 1 && !string.IsNullOrEmpty(Request.Form["ParentUser_Hid"]))
                {
                    info.ParentUserID = CheckParentUser(Request.Form["ParentUser_Hid"]).UserID.ToString();
                }
                //邀请码推荐用户,高于填写的推荐人
                ...
                if (!string.IsNullOrEmpty(inviteCode))
                {
                    ...
                    else { function.WriteErrMsg("邀请码[" + inviteCode + "]无效");return null; }
                }
                ////计算深度
        ...
        #endregion
        public ActionResult ASCXHtml(string ascx)
        {
            switch (ascx)
            {
                case "UserShopTop":
