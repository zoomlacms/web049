...
namespace ZoomLaCMS.Areas.User.Controllers
{
    public class ExamController : ZLCtrl
    {
        ...
        public ActionResult AddEngLishQuestion()
        {
            ...
            if (mu.UserID != model.questMod.UserID) {function.WriteErrMsg("你无权限修改该试题"); }
        ...
        {
            int issmall = DataConverter.CLng(Request.QueryString["issmall"]);
        ...
        {
            PageSetting setting = buser.SelByGroupType_SPage(CPage, PSize, "isteach", Request["skey"]);
        ...
        {
            if (!buser.IsTeach() && !badmin.CheckLogin()) { function.WriteErrMsg("当前页面只有教师才可访问"); return null; }
            VM_ExamDetail model = ExamDetail_MyBind();
            if (model == null) { function.WriteErrMsg("试卷不存在,可能已被删除"); return null; }
            return View(model);
        }
