...
namespace ZoomLaCMS.Areas.User.Controllers
{
    public class ChangeController : Ctrl_Guest
    {
        ...
        //直接修改手机号,需要后端配置
        public ActionResult Mobile_Free()
        {
            if (!SiteConfig.UserConfig.UserMobilAuth.Equals("1")) { function.WriteErrMsg("未允许自由修改手机号");return null; }
        ...
        public string UserMobile { get {return DataConverter.CStr(Session["GetPwd_UserMobile"]); } set { Session["GetPwd_UserMobile"] = value; } }
        ...
        public ActionResult GetPassWord_Mobile()
        {
            ...
            if (mobileBll.CheckMobile(mobile))
            {
                ...
                //添加一条发送手机短信记录
                mobileBll.Insert(new M_Safe_Mobile() { Phone = mobile, VCode = code, Source = "GetPassWord_Mobile", UserID = mu.UserID, UserName = mu.UserName,SysRemind=ret });
            }
            else
            {
        ...
        //验证手机校验码,返回密码修改页
        public ActionResult GetPassword_Mobile_Code()
        {
            string code = Request.Form["CheckCode_T"];
            ...
            {
                function.WriteErrMsg(retMod.err); return null;
            }
        }
        public void GetPassWord_Email()
        {
            ...
            function.WriteSuccessMsg("密码重设请求提交成功,<a href='" + emailUrl + "' target='_blank'>请前往邮箱查收</a>!!", "", 0); return;
        }
