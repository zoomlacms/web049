...
namespace ZoomLaCMS.Areas.AdminMVC.Controllers
{
    public class CRMController : Ctrl_Admin
    {
        ...
        {
            if (!B_ARoleAuth.AuthCheckEx(ZLEnum.Auth.office, "crm")) { return null; }
            PageSetting setting = clientBll.SelPage(CPage, PSize, new F_CRMS_Client() { ctype = Request["ctype"] });
            if (function.isAjax())
            {
        ...
        {
            if (Mid < 1) { function.WriteErrMsg("未指定客户信息");return null; }
            if (!B_ARoleAuth.AuthCheckEx(ZLEnum.Auth.office, "crm")) { return null; }
        ...
        {
            if (!B_ARoleAuth.AuthCheckEx(ZLEnum.Auth.office, "crm")) { return; }
        ...
        public ActionResult ClientImport()
        {
            return View();
        }
        ...
        public void Import_Client()
        {
            ...
            if (file == null) { function.WriteErrMsg("未指定文件"); return; }
            if (file.ContentLength < 100) { function.WriteErrMsg("文件为空"); return; }
            string ext = Path.GetExtension(file.FileName).ToLower();
            if (ext != ".xlsx") { function.WriteErrMsg("只允许上传xlsx文件"); return; }
        ...
        public ActionResult Contact()
        {
            ...
            if (function.isAjax()) { return PartialView("Contact_List", setting); }
        ...
        public ActionResult ContactImport() {
            if (!B_ARoleAuth.AuthCheckEx(ZLEnum.Auth.office, "crm")) { return null; }
            return View();
        }
        ...
        public void ContactImport_Upload()
        {
            if (!B_ARoleAuth.AuthCheckEx(ZLEnum.Auth.office, "crm")) { return; }
            var file = Request.Files["file_up"];
            if (file == null) { function.WriteErrMsg("未指定文件"); return; }
            ...
            string ext = Path.GetExtension(file.FileName).ToLower();
            if (ext != ".xlsx") { function.WriteErrMsg("只允许上传xlsx文件"); return; }
            DataTable excelDT = Import_Logical.Excel_ToDT(XMLPath_Contact, file.InputStream);
            //将数据直接写入数据库,或将其转换为模型再写入
            foreach (DataRow dr in excelDT.Rows)
            {
            ...
            function.WriteSuccessMsg("联系人导入成功", "ClientImport");
        }
        #endregion        
    }
}
